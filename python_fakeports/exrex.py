...
class ErexArgs(object):
    ...
    def has(self,c):
        if c in [ sre_parse.CATEGORY_DIGIT, sre_parse.CATEGORY_WORD ] and len(self.args) > 0:
            return True
        return False

    def get_arg(self):
        return str(self.args.pop())
...
def _gen(d, limit=20, count=False, grouprefs=None, args=None):
    """docstring for _gen"""
...
def generate(s, limit=20, args=None):
    ...
    """
    if args is not None:
        args = ErexArgs(list(args))
    return _gen(parse(s), limit, args=args)
...
def getone(regex_string, limit=20, args=None):
    """Returns a random matching string to a given regular expression
    """
    if args is not None:
        args = ErexArgs(list(args))
    return _randone(parse(regex_string), limit, args=args)
